...
public class ControladorLES {
	...
	public void agregarCant(String ID) {
		ArrayList<Servicio> list = Biblioteca.getServicios();
		for (Servicio servicio : list) {
			if (servicio.getID().equals(ID)) {
				int num = servicio.getCantidad();
				servicio.setCantidad(num+1);
				try {
	                Escritor.escribirServicio("./Servicios/Servicio.txt", list);
	            } catch (IOException e) {
	                e.printStackTrace();
	            }
			}
	...
	public void eliminarCant(String ID) {
		ArrayList<Servicio> list = Biblioteca.getServicios();
		for (Servicio servicio : list) {
			if (servicio.getID().equals(ID)) {
				int num = servicio.getCantidad();
